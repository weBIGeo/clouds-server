...
// =============================================================================
// PROCEDURAL DENSITY FACTOR
// =============================================================================

// Maps NWP cloud fraction and TKE to a volumetric density multiplier.
// Stratus path (low TKE): smooth slab with gentle interior brightness variation.
// Convective path (high TKE): Perlin-shaped footprint with Worley surface texture.
// conv_weight blends continuously between the two paths.
fn compute_density_factor(p_warped: vec3<f32>, clc: f32, tke: f32) -> f32 {
    // --- Convective Weighting (Altitude & TKE Aware) ---
    // Tightened standard ramp: requires a baseline of turbulence to trigger cumulus.
    let tke_cu = smoothstep_custom(0.05, 0.5, tke);
    
    // Altitude fade: standard cumulus dies out between 5,000m and 8,000m.
    let alt_fade = 1.0 - smoothstep_custom(5000.0, 8000.0, p_warped.z);
    
    // Cumulonimbus bypass: Extreme TKE (> 1.2) forces the convective path 
    // regardless of altitude, preserving boiling thunderstorm tops.
    let tke_cb = smoothstep_custom(0.8, 1.5, tke);
    
    // Final weight: Standard cumulus (faded by altitude) OR Cumulonimbus.
    let conv_weight = max(tke_cu * alt_fade, tke_cb);

    // --- Noise ---
    // Stratus: interior brightness and large-scale edge perturbation.
    let n_strat_interior = fbm_gradient(
        p_warped / vec3<f32>(SCALE_STRAT_XY, SCALE_STRAT_XY, SCALE_STRAT_Z), 2);
    let n_strat_edge     = fbm_gradient(
        p_warped / vec3<f32>(SCALE_PERTURB_XY, SCALE_PERTURB_XY, SCALE_PERTURB_Z), 2) - 0.5;

    // Convective: structural shape (gradient noise only for connected blobs).
    // Large scale is 2200m to ensure isolated islands at low CLC are >600m across.
    let n_shape_large = fbm_gradient(p_warped / vec3<f32>(2200.0, 2200.0, 1500.0), 3) - 0.5;
    let n_shape_mid   = fbm_gradient(p_warped / vec3<f32>( 800.0,  800.0,  400.0), 3) - 0.5;
    
    // Convective: surface texture (Web + Mid Worley + High-Freq Worley).
    let n_web       = fbm_gradient(p_warped / vec3<f32>(1500.0, 1500.0, 2000.0), 4);
    let n_worley    = fbm_worley(  p_warped / vec3<f32>( 600.0,  600.0, 1300.0), 3);
    let n_worley_hf = fbm_worley(  p_warped / vec3<f32>( 150.0,  150.0,  150.0), 3);

    // --- Boundary perturbation ---
    // Stratus: edge-concentrated via boundary mask (peaks at clc = 0.5).
    let boundary_mask = clc * (1.0 - clc) * 4.0;
    let strat_perturb = n_strat_edge * 0.4 * boundary_mask;

    // Convective: large structural displacement, gated on clc to suppress clear-air noise.
    // We scale the zero-mean noise by 1.5. Because of the asymmetrical saturate() below, 
    // this deep swing mathematically creates the exact topology you requested:
    // - High CLC (0.9): Evaluates to [0.0, 1.0] -> Stays completely solid/connected.
    // - Med CLC (0.5): Evaluates to [-0.3, 1.0] -> Carves deep 0.0 voids/cracks.
    // - Low CLC (0.2): Evaluates to [-0.6, 0.7] -> Mostly 0.0, leaving massive isolated islands.
    let conv_noise_gate = smoothstep_custom(0.0, 0.15, clc);
    let conv_perturb    = (n_shape_large * 0.7 + n_shape_mid * 0.4) * conv_noise_gate * 1.5;

    let blended_perturb = lerp(strat_perturb, conv_perturb, conv_weight);
    let base_shape      = clc + blended_perturb;

    // Edge sharpness tightens with conv_weight. The saturate() before smoothstep
    // discards positive perturbations on dense cells, asymmetrically favouring
    // erosion over growth. This is intentional: it reproduces the fraying and
    // scalloping observed at convective cloud boundaries.
    let edge_upper = lerp(1.0, 0.5, conv_weight);
    let clc_shaped = smoothstep_custom(0.0, edge_upper, saturate(base_shape));

    // --- Texture ---
    // Stratus: brightness in [0.6, 1.0]. Does not punch holes.
    let tex_strat = n_strat_interior * 0.4 + 0.6;

    // Convective: cauliflower bumps from Web x Worley x HF-Worley.
    // Expected mean of multiplying these three noises is ~0.125.
    let raw_texture = n_web * n_worley * n_worley_hf;
    
    // Center at 1.0 (mass conserving), amplify contrast (x6.0) for deep lighting shadows, 
    // and floor at 0.3 to physically prevent interior voids or the "stucco" look.
    let tex_cauliflower = max(1.0 + (raw_texture - 0.125) * 6.0, 0.3);
    
    // High TKE attenuates structure toward the fully turbulent breakdown regime.
    let conv_surface = tex_cauliflower * (1.0 - saturate(tke * 0.5) * 0.3);
    
    // Blend toward neutral (1.0) in the cloud core to prevent interior voids.
    let core_mask = smoothstep_custom(0.2, 0.8, clc_shaped);
    let tex_conv  = lerp(conv_surface, 1.0, core_mask);

    let tex_final = lerp(tex_strat, tex_conv, conv_weight);

    return clc_shaped * tex_final;
}
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Scale factors to convert a world-space displacement (meters) to source-grid texels.
    let disp_to_src_x = params.inv_scale_x / (cos_lat * params.res_h_m);
    let disp_to_src_y = params.inv_scale_y / (cos_lat * params.res_h_m);

    for (var z = 0; z < nz_out; z++) {
        let height  = (f32(z) + 0.5) * params.res_v_m;
        ...
        let disp     = compute_displacement(p_world, env_tke);
        let p_warped = p_world + disp;

        let mat_src_x  = src_x  + disp.x * disp_to_src_x;
        let mat_src_y  = src_y  - disp.y * disp_to_src_y;
        let mat_height = height + disp.z;
        var mat = sample_material(mat_src_x, mat_src_y, mat_height, nx_s, ny_s, nz_s, nz_h);
        ...
        var result = 0.0;
        if (clc_eff > 0.0) {
            let density_factor = compute_density_factor(p_warped, clc_eff, env_tke);
            if (density_factor > 0.0) {
                result = compute_extinction(mat, clc_eff) * min(density_factor, 2.0);
            }
        }
